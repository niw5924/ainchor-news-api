...
router.post('/summary', async (req, res) => {
  try {
    const { text, summaryCount } = req.body;
    console.log('[summary] text:', text);
    console.log('[summary] summaryCount:', summaryCount);

    const result = await openai.responses.create({
      model: 'gpt-4o-mini',
      input: '제공된 내용을 바탕으로 핵심 정보만 추출하여, 뉴스에서 정보를 전달하는 뉘앙스로 한국어 '
        + summaryCount
        + '문장으로 작성: '
        + text
    });
...
router.post('/tts', async (req, res) => {
  try {
    ...
    console.log('[tts] anchorName:', anchorName);
    console.log('[tts] summary:', summary);
